title GoAnimate 2014 Settings Script
...
if not exist start_goanimate.bat ( goto error_location )
goto noerror_location
:error_location
echo Doesn't seem like this script is in the GoAnimate 2014 folder.
goto end
:noerror_location
...
if "!choice!"=="?1" (
	echo When enabled, an extra window opens with more info about what GoAnimate 2014 is doing.
...
if "!choice!"=="?3" (
	...
	if !CUSTOMBROWSER!==y (
		echo This setting defines which browser GoAnimate 2014 launches with when set to use a custom browser.
...
if "!choice!"=="?4" (
	...
	echo This is automatically disabled when GoAnimate 2014 launches and finds all dependencies.
...
if "!choice!"=="?6" (
	...
	echo Turning this off will stop GoAnimate 2014 from auto-updating.
	goto reaskoptionscreen
)
:: Waveforms
...
if "!choice!"=="?8" (
    ...
    echo:
	echo It's used to show when you're using GoAnimate 2014 in your "Playing A Game" status on Discord,
...
if exist "server\characters\characters.zip" (
	...
	if "!choice!"=="?9" (
		echo When first getting GoAnimate 2014, all non-stock characters are put into a single zip file.
...
echo:
echo:
echo Press 1 to use GA2014's included Chromium (recommended)
